...
export {CCW,Track,ChainSword,PowerSword};
export {TwinLinked,OnVehicle}
function TwinLinked(weapon)
{
    weapon.name='Twin-Linked '+weapon.name;
    weapon.attack=weapon.attack*2;
    return weapon;
}
function OnVehicle(weapon)
{
    if (weapon.rules===null)
    {
        weapon.rules=[];
    }
    weapon.rules.push('Vehicle');
    return weapon;
}
function Weapon(option)
{
    ...
    this.attack=option.attack || 0;
    this.damage=option.damage || 0;
 ...
 * Vehicle
 */
let Track=new Weapon({
    name:'Track',
    description:'Vehicle try RAM and crush enemy with trucks',
    cost:0,
    range:0,
    strength:0,
    rending:0,
